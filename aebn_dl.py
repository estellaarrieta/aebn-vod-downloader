...
def modify_manifest(manifest_content, base_url, duration_seconds, target_height):
    ...
    # Remove streams that do not match the target height
    ...
    return modified_manifest_content
...
def movie_scrape(MOVIE_URL):
    ...
    content = html.fromstring(session.get(MOVIE_URL).content)
    movie_id = MOVIE_URL.split("/")[5]
    manifest_url = get_manifest_url(movie_id, session)
...
def get_manifest_url(movie_id, session):
    data = f"movieId={movie_id}&isPreview=true&format=DASH"
    content = session.post("https://straight.aebn.com/straight/deliver", data=data).json()
...
def main(MOVIE_URL):
    ...
    modified_manifest_content = modify_manifest(manifest_content, base_url, duration_seconds, target_height)
    name = f"{name} {target_height}p"
    print(name)
